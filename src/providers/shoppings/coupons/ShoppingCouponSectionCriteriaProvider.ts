...
export namespace ShoppingCouponSectionCriteriaProvider {
  ...
  export const collect =
    ...
    async (input: IShoppingCouponSectionCriteria.ICreate) => {
      const sectionList =
        await ShoppingGlobal.prisma.shopping_sections.findMany({
          where: {
            code: {
              in: input.section_codes,
            },
          },
        });
      if (sectionList.length !== input.section_codes.length)
