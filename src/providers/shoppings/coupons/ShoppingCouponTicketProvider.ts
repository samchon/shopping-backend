...
export namespace ShoppingCouponTicketProvider {
  ...
  export const index =
    ...
    async (
      ...
      } satisfies Prisma.shopping_coupon_ticketsFindManyArgs)(input);
    };
  ...
  export const at =
    (customer: IShoppingCustomer) =>
    async (id: string): Promise<IShoppingCouponTicket> => {
      authorize(customer);
      const record =
  ...
  export const create =
    ...
    async (
      ...
      const coupon: IShoppingCoupon = await ShoppingCouponProvider.at(customer)(
        input.coupon_id,
      );
      if (coupon.inventory.volume !== null && coupon.inventory.volume < 1)
        ...
        throw ErrorProvider.gone({
          accessor: "input.coupon_id",
          message:
            "The number of coupons issued per citizen has been exceeded.",
        });
      else if (
