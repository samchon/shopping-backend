...
export namespace ShoppingSaleSnapshotUnitStockProvider {
  export namespace json {
    export const transform = (
      input: Prisma.shopping_sale_snapshot_unit_stocksGetPayload<ReturnType<typeof select>>
    ): IShoppingSaleUnitStock => ({
      id: input.id,
      name: input.name,
      choices: input.choices.sort((a, b) => a.sequence - b.sequence).map(ShoppingSaleSnapshotUnitStockChoiceProvider.json.transform),
      inventory: {
  ...
  export const collect = 
    ...
    (
      ...
      sequence,
      choices: {
        create: input.choices.map(ShoppingSaleSnapshotUnitStockChoiceProvider.collect(optionList))
      },
