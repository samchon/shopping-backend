...
export namespace ShoppingSaleSnapshotUnitOptionProvider {
  export namespace json {
    export const transform = (
      input: Prisma.shopping_sale_snapshot_unit_optionsGetPayload<ReturnType<typeof select>>
      ...
      input.type === 'select'
        ? {
          ...
          candidates: input.candidates.sort((a, b) => a.sequence - b.sequence).map(ShoppingSaleSnapshotUnitOptionCandidateProvider.json.transform),
        }
        : {
          id: input.id,
    ...
    export const select = () => ({
      include: {
        candidates: ShoppingSaleSnapshotUnitOptionCandidateProvider.json.select(),
      }
    ...
    ({
      ...
      candidates: {
        create: 
          input.type === 'select' 
            ? input.candidates
                .map(ShoppingSaleSnapshotUnitOptionCandidateProvider.collect)
            : []
      },
      sequence,
