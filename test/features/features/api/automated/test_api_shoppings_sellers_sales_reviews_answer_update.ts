import typia from "typia";
import type { Format } from "typia/lib/tags/Format";

import api from "../../../../../src/api";
import type { IBbsArticle } from "../../../../../src/api/structures/common/IBbsArticle";
import type { IShoppingSaleInquiryAnswer } from "../../../../../src/api/structures/shoppings/sales/inquiries/IShoppingSaleInquiryAnswer";

export const test_api_shoppings_sellers_sales_reviews_answer_update = async (
  connection: api.IConnection,
) => {
  const output: IShoppingSaleInquiryAnswer.ISnapshot =
    await api.functional.shoppings.sellers.sales.reviews.answer.update(
      connection,
      {
        saleId: typia.random<string & Format<"uuid">>(),
        reviewId: typia.random<string & Format<"uuid">>(),
        body: typia.random<IBbsArticle.ICreate>(),
      },
    );
  typia.assert(output);
};
